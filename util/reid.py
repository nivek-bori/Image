...
from util.config import ReidConfig
...
# factory reid
class Reid:
    def __init__(self, reid_config=None):
        if reid_config is None:
            reid_config = ReidConfig()

        self.type = reid_config.type
        match self.type:
            case 'ave':
                self._reid = Ave_Reid(reid_config.max_lookback, reid_config.shape)
            case 'time':
                self._reid = Time_Reid(
                    reid_config.max_lookback, reid_config.shape, mult=reid_config.mult
                )
            case 'weight':
                self._reid = Weight_Reid(reid_config.max_lookback, reid_config.shape)
            case _:
                raise ValueError(
                    f'Unknown reid type {type}. Valid types are ave, time, weight'
                )
...
class Time_Reid:
    def __init__(self, max_num, reid_shape, mult):
        self.max_num = max_num
        self.mult = mult
        self.max_single_weight = self.calculate_single_weight(
            self.max_num
        )  # maximum weight on single value
    ...
    def step_reid(self, reid):
        ...
        self.sum -= (
            self.reids[self.head] * self.max_single_weight
        )  # remove currrent weighted reid
    ...
    def get_reid(self):
        ...
        self.sum_weight = (
            ...
            else self.calculate_weight(self.num)
        )
