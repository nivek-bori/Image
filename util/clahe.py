...
def calculate_cdf(image, clip_max, luminance_idx=0):
    hist = np.bincount(
        image[:, :, luminance_idx].flatten(), minlength=256
    )  # calculate histogram

    clip_total = np.sum(
        np.maximum(0, hist - clip_max)
    )  # clip_total is sum of excess values
...
def self_interpolate(image, cdfs, luminance_idx, image_shape, grid_shape, tile_size):
    ...
    for y in range(height):
        for x in range(width):
            ...
            frac_y, frac_x = (y % y_step) / (y_step - 1), (x % x_step) / (
                x_step - 1
            )  # xy position in tile
            cdist_y, cdist_x = abs(frac_y - 0.5), abs(
                frac_x - 0.5
...
def bilinear_interpolate(
    image, cdfs, luminance_idx, image_shape, grid_shape, tile_size
):
    output = image.copy()
    ...
    for y in range(height):
        for x in range(width):
            ...
            t_y, t_x = int((y - y_step / 2) / y_step), int(
                (x - x_step / 2) / x_step
            )  # tile position
            frac_y, frac_x = (y % y_step) / (y_step - 1), (x % x_step) / (
                x_step - 1
...
def apply_self_clahe(
    ...
    output = bilinear_interpolate(
        image, cdfs, luminance_idx, (height, width), grid_shape, (y_step, x_step)
    )
...
def test_clahe(
    ...
    lab_ineffective_frame = cv2.cvtColor(
        np.array(IMAGE2LAB(ineffective_frame, 'bgr')[0][:, :, 0]), cv2.COLOR_GRAY2BGR
    )
    self_clahe_frame = apply_self_clahe(
        ineffective_frame,
        0.2,
        image_type='bgr',
    ...
    opencv_clahe_frame = apply_opencv_clahe(
        ineffective_frame, 2.0, grid_shape=grid_shape
    )
...
def test_self_interpolation(grid_shape=(2, 2), image_size=(400, 400), print_flag=False):
    ...
    grey_values = np.linspace(
        30, 220, total_tiles, dtype=np.uint8
    )  # Spread from dark to light
